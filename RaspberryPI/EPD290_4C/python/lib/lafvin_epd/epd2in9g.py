...
class EPD:
    ...
    def _configure_panel(self):
        ...
        # 每一段都先发送命令，再发送该命令对应的数据。
        # 更换屏幕型号时，需要使用新屏幕对应的初始化参数。
    ...
    def getbuffer(self, image, dither=True):
        """Convert a 128x296 or 296x128 Pillow image to a 9472-byte frame."""
        if image.size == (self.width, self.height):
            ...
            raise ValueError(
                ...
                f"{self.width}x{self.height} or {self.height}x{self.width}"
            )

        dither_mode = DITHER_FLOYDSTEINBERG if dither else DITHER_NONE
        image_4color = image_temp.convert("RGB").quantize(
        ...
        for source in range(0, len(color_codes), 4):
            ...
            pixels = [value if value < 4 else 0 for value in color_codes[source : source + 4]]
            frame[source // 4] = (
                (pixels[0] << 6)
                | (pixels[1] << 4)
                | (pixels[2] << 2)
    ...
    def display(self, image):
        """Write one packed frame to controller RAM and refresh the panel."""
        frame = bytes(image)
        if len(frame) != FRAME_BYTES:
            raise ValueError(f"display buffer must contain exactly {FRAME_BYTES} bytes")
    ...
    def _packed_clear_color(color):
        ...
        if isinstance(color, int):
            ...
            if color in (0x55, 0xAA, 0xFF):
                return color
        raise ValueError("clear color must be a four-color RGB value or 2-bit color code")
    ...
    def Clear(self, color=0x55):
        """Compatibility alias for existing Waveshare-style examples."""
        self.clear(color)
